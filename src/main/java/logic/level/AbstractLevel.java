...
public abstract class AbstractLevel extends Observable implements Level, Observer {
    ...
    AbstractLevel(String name, int numberOfBricks, double probOfGlass, double probOfMetal, double probOfSuper, int seed){
        this.levelName = name;
        this.scoreLevel = 0;
        this.currentScore = 0;
        ...
        double randomNumber = rand.nextDouble();
        if(randomNumber <= probOfSuper){
            SuperBrick sBrick = new SuperBrick();
            sBrick.subscribe(this);
            this.bricksList.add(sBrick);
        }
    }
