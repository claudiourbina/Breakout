...
public class HomeworkTwoFacade {
     ...
     *
     * @param name           the name of the level
     * @param level          the number of the level created
     * @return a new level determined by the parameters
     * @see Level
     */
    public Level newRandomLevel(String name, int level) {
        int seed = (int) System.currentTimeMillis();
        Random rand = new Random(seed);
        int numberOfBricks = (int) (10*level*rand.nextDouble());
        if(numberOfBricks <= 0)
            numberOfBricks = 5;
        double probOfGlass = 2*rand.nextDouble()/level;
        double probOfMetal = level*rand.nextDouble()*rand.nextDouble();
        double probOfSuper = level*rand.nextDouble()*rand.nextDouble();
        return new RealLevel(name, numberOfBricks, probOfGlass, probOfMetal, probOfSuper, seed);
    }
